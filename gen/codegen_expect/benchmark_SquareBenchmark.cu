...
timeStartMeasure();
    ...
    checkErrorReturn(program_result, cudaMalloc(&_kernel_result_6, (sizeof(int) * 10000)));
    program_result->device_allocations->push_back(_kernel_result_6);
    int * _kernel_result_6_host = (int *) malloc((sizeof(int) * 10000));
    kernel_5<<<40, 256>>>(dev_env, 10000, _kernel_result_6);
    checkErrorReturn(program_result, cudaPeekAtLastError());
    checkErrorReturn(program_result, cudaThreadSynchronize());

    checkErrorReturn(program_result, cudaMemcpy(_kernel_result_6_host, _kernel_result_6, (sizeof(int) * 10000), cudaMemcpyDeviceToHost));
