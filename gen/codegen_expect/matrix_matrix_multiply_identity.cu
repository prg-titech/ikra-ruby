...
timeStartMeasure();
    ...
    checkErrorReturn(program_result, cudaMalloc((void **) &host_env->l2_a, 400));
    checkErrorReturn(program_result, cudaMemcpy(host_env->l2_a, temp_ptr_l2_a, 400, cudaMemcpyHostToDevice));

    void * temp_ptr_l2_b = host_env->l2_b;
    checkErrorReturn(program_result, cudaMalloc((void **) &host_env->l2_b, 400));
    checkErrorReturn(program_result, cudaMemcpy(host_env->l2_b, temp_ptr_l2_b, 400, cudaMemcpyHostToDevice));
...
timeStartMeasure();
    ...
    checkErrorReturn(program_result, cudaMalloc(&_kernel_result_2, (sizeof(int) * 100)));
    int * _kernel_result_2_host = (int *) malloc((sizeof(int) * 100));
    kernel_3<<<1, 100>>>(dev_env, 100, _kernel_result_2);
    checkErrorReturn(program_result, cudaPeekAtLastError());
    checkErrorReturn(program_result, cudaThreadSynchronize());

    checkErrorReturn(program_result, cudaMemcpy(_kernel_result_2_host, _kernel_result_2, (sizeof(int) * 100), cudaMemcpyDeviceToHost));
