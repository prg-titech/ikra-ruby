...
extern "C" EXPORT int *launch_kernel(environment_t *host_env)
{
    ...
    checkCudaErrors(cudaMalloc((void **) &host_env->l1_a, 400));
    checkCudaErrors(cudaMemcpy(host_env->l1_a, temp_ptr_l1_a, 400, cudaMemcpyHostToDevice));

    void * temp_ptr_l1_b = host_env->l1_b;
    checkCudaErrors(cudaMalloc((void **) &host_env->l1_b, 400));
    checkCudaErrors(cudaMemcpy(host_env->l1_b, temp_ptr_l1_b, 400, cudaMemcpyHostToDevice));
    ...
    int *device_result;
    checkCudaErrors(cudaMalloc(&device_result, sizeof(int) * 100));
    ...
    checkCudaErrors(cudaMemcpy(host_result, device_result, sizeof(int) * 100, cudaMemcpyDeviceToHost));
